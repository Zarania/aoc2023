...
fn cycle_length(key: &str, nodes: &HashMap<&str, [&str; 2], BuildHasherDefault<FxHasher>>, steps: &str, target: &str) -> u32 {
    let mut key = key;
    steps.chars().map(|s| match s {
        'L' => 0,
        'R' => 1,
        _ => unreachable!()
    }).cycle().enumerate().find(|&(_, step)| {
...
pub fn part_two(input: &str) -> Option<usize> {
    ...
    Some(nodes.iter().filter(|(key, _)| key.ends_with('A')).map(|k| cycle_length(k.0, &nodes, steps, "Z") as usize).fold(1, |a, b| {
        (a * b) / gcd(a, b)
    }))
}
